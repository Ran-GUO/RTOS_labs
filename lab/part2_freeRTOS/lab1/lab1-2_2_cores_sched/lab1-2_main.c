...
void app_main(void) {
...
#ifdef IDLE_HOOKS
      esp_register_freertos_idle_hook_for_cpu(vApplicationIdleHook_0, 0);
      esp_register_freertos_idle_hook_for_cpu(vApplicationIdleHook_1, 1);
#endif
	...
	/* Create tasks. */
		...
		xTaskCreatePinnedToCore(vTaskFunction,	"Task 1", STACK_SIZE,	(void*)"Task 1", T1_PRIO,	NULL,0);					
		xTaskCreatePinnedToCore(vTaskFunction, "Task 2", STACK_SIZE, (void*)"Task 2", T2_PRIO, NULL,0);
	#elif PINNED_TO_CORE == 0x01
		xTaskCreatePinnedToCore(vTaskFunction,	"Task 1", STACK_SIZE,	(void*)"Task 1", T1_PRIO,	NULL,0);					
		xTaskCreatePinnedToCore(vTaskFunction, "Task 2", STACK_SIZE, (void*)"Task 2", T2_PRIO, NULL,1);
	#elif PINNED_TO_CORE == 0x10
		xTaskCreatePinnedToCore(vTaskFunction,	"Task 1", STACK_SIZE,	(void*)"Task 1", T1_PRIO,	NULL,1);					
		xTaskCreatePinnedToCore(vTaskFunction, "Task 2", STACK_SIZE, (void*)"Task 2", T2_PRIO, NULL,0);	
	#elif PINNED_TO_CORE == 0x11
		xTaskCreatePinnedToCore(vTaskFunction,	"Task 1", STACK_SIZE,	(void*)"Task 1", T1_PRIO,	NULL,1);					
		xTaskCreatePinnedToCore(vTaskFunction, "Task 2", STACK_SIZE, (void*)"Task 2", T2_PRIO, NULL,1);		
		xTaskCreatePinnedToCore(vTaskFunction, "Task 3", STACK_SIZE, (void*)"Task 3", T3_PRIO, NULL,0);		
	#else
