...
const uint32_t INSPECTOR_TASK_CORE = 0;
...
void vTaskInspector(void *pvParameters) {
	...
	DISPLAY("Task Inspector is checking");
		...
		for(int index=1; index<TABLE_SIZE;index++ ){
			COMPUTE_IN_TICK (0.01) ;
			if(Table[index] !=(reference+index)) error=true;			
		}
		DISPLAY("Task Inspector ended its checking.");

		if(error ==true){
			DISPLAY(TAG, "Consistency error in the Table variable.");
			break;
		}
